 ...
 */
final class TaxRatesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private TaxRateRepository $taxRateRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'tax_rates' => $this->taxRateRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\TaxRates\ModuleException("Record {$idStr} not found for locking.");
            }
