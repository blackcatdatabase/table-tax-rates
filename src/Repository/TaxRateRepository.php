...
use BlackCat\Database\Packages\TaxRates\Repository\TaxRateRepositoryInterface;
...
class TaxRateRepository implements TaxRateRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'country_iso2', 'category', 'valid_from' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\TaxRates\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'rate', 'valid_to' ]);
          return count($rows);
        }
    ...
    public function getByCountryIso2AndCategoryAndValidFrom(string $countryIso2, string $category, \DateTimeInterface|string $validFrom, bool $asDto = false): array|\BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto|null {
        return $this->getByUnique([ 'country_iso2' => $countryIso2, 'category' => $category, 'valid_from' => $validFrom ], $asDto);
    }
    ...
    public function getIdByCountryIso2AndCategoryAndValidFrom(string $countryIso2, string $category, \DateTimeInterface|string $validFrom) {
        $row = $this->getByCountryIso2AndCategoryAndValidFrom($countryIso2, $category, $validFrom, false);
