...
namespace BlackCat\Database\Packages\TaxRates\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\TaxRates\Definitions;
use BlackCat\Database\Packages\TaxRates\Criteria;
use BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto as Dto;
use BlackCat\Database\Packages\TaxRates\Mapper\TaxRateDtoMapper as RowMapper;
...
final class TaxRateRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\TaxRates\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'country_iso2', 'category', 'valid_from' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\TaxRates\Definitions::table());
            return $bulk->upsertMany($rows, [ 'country_iso2', 'category', 'valid_from' ], [ 'rate', 'valid_to' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto|null */
    public function getByCountryIso2AndCategoryAndValidFrom(string $countryIso2, string $category, \DateTimeInterface|string $validFrom, bool $asDto = false): array|\BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto|null {
        $row = $this->getByUnique([ 'country_iso2' => $countryIso2, 'category' => $category, 'valid_from' => $validFrom ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\TaxRates\Mapper\TaxRateDtoMapper::fromRow($row);
    }
    public function existsByCountryIso2AndCategoryAndValidFrom(string $countryIso2, string $category, \DateTimeInterface|string $validFrom): bool {
        $where = 't.' . Ident::q($this->db, 'country_iso2') . ' = :uniq_country_iso2' . ' AND ' . 't.' . Ident::q($this->db, 'category') . ' = :uniq_category' . ' AND ' . 't.' . Ident::q($this->db, 'valid_from') . ' = :uniq_valid_from';
        return $this->exists($where, [ 'uniq_country_iso2' => $countryIso2, 'uniq_category' => $category, 'uniq_valid_from' => $validFrom ]);
    }
    /** @return int|string|null */
    public function getIdByCountryIso2AndCategoryAndValidFrom(string $countryIso2, string $category, \DateTimeInterface|string $validFrom) {
        $row = $this->getByCountryIso2AndCategoryAndValidFrom($countryIso2, $category, $validFrom);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\TaxRates\Dto\TaxRateDto|null {
